...
namespace xx {
	...
	struct AudioItemWithDecoder {
		...
		Shared<bool> pauseFlag, stopFlag;
	};
	...
	struct AudioDevice {
		...
		AudioDevice() {
			...
			if (ma_device_init(nullptr, &deviceConfig, &device) != MA_SUCCESS) {
				...
				CoutN("Failed to init ma device.");
#endif
				throw - 1;
			}
			if (ma_device_start(&device) != MA_SUCCESS) {
#ifdef AUDIO_SHOW_CONSOLE_LOG
				CoutN("Failed to start ma device.");
#endif
				throw - 2;
			}
		}
	...
	struct Audio {
		Listi32<Shared<AudioDevice>> devices;	// pool
		...
		Shared<bool> pauseFlag;			// need init
		Shared<bool> stopFlag;			// need init

		Tasks maTasks;
		Listi32<AudioItem> tmpItems;
		...
		// volume = 0 ~ 1
		void Play(DataShared d, float volume = 1, bool isLoop = false) {
			for (auto& o : tmpItems) {
				if (o.data.h == d.h) {
					++o.count;
					return;
		...
		void Update() {
			for (auto& o : tmpItems) {
				maTasks.Add(([&](AudioItem o)->Task<> {
					// alloc device
					if (devices.Empty()) co_return;
					auto d = std::move(devices.Back());
					devices.PopBack();
					auto sgDevice = MakeScopeGuard([&] {
						devices.Add(std::move(d));
#ifdef AUDIO_SHOW_CONSOLE_LOG
						CoutN("devices.Add(std::move(d));");
#endif
						});
					...
					auto sgDecoder = MakeScopeGuard([&] {
						ma_decoder_uninit(&decoder);
#ifdef AUDIO_SHOW_CONSOLE_LOG
						CoutN("ma_decoder_uninit(&decoder);");
#endif
						});

					if (r != MA_SUCCESS) {
#ifdef AUDIO_SHOW_CONSOLE_LOG
						CoutN("error init decoder");
#endif
						co_return;
					}
