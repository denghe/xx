...
namespace xx::TMX {
	...
	struct Filler {
		...
		void TryFillImage(Ref<Image>& out, pugi::xml_node const& c);
		...
		void TryFillObject(Ref<Object>& o, pugi::xml_node const& c);
	...
	inline static void FillBinIntsTo(std::vector<T>& out, Data_r dr) {
	...
	inline static void TryFill(T& out, pugi::xml_attribute const& a) {
		...
		if constexpr (std::is_same_v<U, std::string>) {
			...
			FillEnumTo(out, a);
		} else if constexpr (IsOptional_v<U>) {
	...
	void Filler::TryFillProperties(std::vector<Property>& out, pugi::xml_node const& owner, bool needOverride) {
		for (auto&& c : owner.child("properties").children("property")) {
			...
			{
				p->value = c.attribute("value").as_llong();	// convert to Weak<Object> at fill step 2
	...
	void Filler::TryFillImage(Ref<Image>& out, pugi::xml_node const& c) {
		out.Reset();
		if (c.empty()) return;
		std::string s;
		TryFill(s, c.attribute("source"));
		auto&& fp = rootPath + s;	// to fullpath
		auto&& iter = std::find_if(map.images.begin(), map.images.end(), [&](Ref<Image> const& img) {
			return img->source == s;
			});
		...
		if (map.infinite) {
			...
			case Encodings::Base64:
			{
				...
				for (auto&& cChunk : cData.children("chunk")) {
					auto&& chunk = L.chunks.emplace_back();
					TryFillChunkBase(chunk, cChunk);
					auto&& bin = Base64Decode(Trim(cChunk.text().as_string()));
					switch (compression) {
			...
			{
				Data tmp;
				auto&& bin = Base64Decode(Trim(cData.text().as_string()));
	...
	void Filler::TryFillObject(Ref<Object>& o, pugi::xml_node const& c) {
